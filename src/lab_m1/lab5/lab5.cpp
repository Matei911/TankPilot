...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    }
    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(80.0f), glm::vec3(1, 0, 0));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
    {
        glm::mat4 modelMatrix = glm::mat4(1);
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-7, 2.5f, 0));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 0, 0));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_C)) {
            // TODO(student): Translate the camera upward
            radians -= deltaTime * cameraSpeed;
            projectionMatrix = glm::perspective(radians, window->props.aspectRatio, 0.01f, 200.0f);
        }
        ...
        if (window->KeyHold(GLFW_KEY_N)) {
            length -= deltaTime * cameraSpeed;
            // TODO(student): Translate the camera upward
            projectionMatrix = glm::ortho(-width, width, -length, length, z_near, z_far);
        }

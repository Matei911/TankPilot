...
{
    polygonMode = GL_FILL;
    std::cout <<RESOURCE_PATH::MODELS <<"box.obj" << std::endl;
    Mesh* mesh = new Mesh("box");
    mesh->LoadMesh(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::MODELS, "primitives"), "tankcolor.obj");
...
void Lab4::RenderScene() {
    ...
    RenderMesh(meshes["asd"], shaders["VertexNormal"], modelMatrix);
...
void Lab4::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_3)) {
        angularStepOX += 100 * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_4)) {
        angularStepOX -= 100 * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_5)) {
        angularStepOY += 100 * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_6)) {
        angularStepOY -= 100 * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_7)) {
        angularStepOZ += 100 * deltaTime;
    }
